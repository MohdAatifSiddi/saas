...
function publicOrigin(name: string, fallback: string): string {
  const value = env(name, fallback);
  const url = new URL(value);
  const isLocal = url.hostname === 'localhost' || url.hostname === '127.0.0.1';
  if (isProduction && (url.protocol !== 'https:' || isLocal)) {
...
const betterAuthUrl = env('BETTER_AUTH_URL', 'http://localhost:3001/api/auth');
const frontendUrl = publicOrigin('FRONTEND_URL', 'http://localhost:3000');
...
export const auth = betterAuth({
  ...
  // Keep Better Auth's route-specific rate limits. For multi-instance deployments,
  // configure shared secondary storage so limits are not process-local.
  session: {
